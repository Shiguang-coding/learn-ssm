> 在线视频：[尚硅谷新版SSM框架全套视频教程，Spring6+SpringBoot3最新SSM企业级开发](https://www.bilibili.com/video/BV1AP411s7D7?p=67)
>
> 在线笔记：[MyBatis实践：提高持久层数据处理效率](https://www.wolai.com/oLP8DUTzo2JRX1DfZNZn6c)
>
...
> [Mybatis简介](https://mybatis.org/mybatis-3/zh_CN/index.html)
...
mybatis框架配置文件： 数据库连接信息，性能配置，`mapper.xml`配置等！
...
public class MyBatisTest {
    ...
    public void testSelectEmployee() throws IOException {
        ...
        // 4.关闭SqlSession
        session.commit(); //提交事务 [DQL(查询)不需要,其他需要]
        session.close(); //关闭会话
...
`openSession()`: 自动开启事务，不会自动提交事务，适用于DQL

`openSession(true)`： 自定开启事务，并自动提交，适用于DML
...
![image-20240904112133556](images/66d7d23d5d8ce.png)
...
/**
 * Created By Shiguang On 2024/9/4 10:45
 */
...
![image-20240904113432980](images/66d7d548cf7e1.png)

对于不同表的操作都需要维护与之相对应的xml文件，里面存储着增删改查的sql语句并配置到核心配置文件中，核心配置文件存储了数据库信息、各功能开关、以及不同表的sql语句，`SqlSessionFactory`会读取核心配置文件中的信息并将这些信息以Map的形式进行存储并生成全局缓存，`SqlSession`执行时根据Map中存储的 key （namespace.id | id）查找相应的Sql语句，拼接参数并执行Sql语句，最后将执行的结果返回。

### Mybatis

![image-20240904115707895](images/66d7da93ecbb4.png)

Mybatis对Ibatis做了进一步封装，通过代理技术（jdk动态代理），使用`getMapper()`方法获取接口的代理对象执行mapper接口方法，执行时根据内部mapper接口的全限定符以 + 方法名称获取Sql语句。
...
| **设置名** |                       **描述**                        | **有效值**                                                   | 默认值 |
| ---------- | :---------------------------------------------------: | ------------------------------------------------------------ | :----- |
| logImpl    | 指定 MyBatis 所用日志的具体实现，未指定时将自动查找。 | SLF4J \| LOG4J(3.5.9起废弃) \| LOG4J2 \| JDK_LOGGING \| COMMONS_LOGGING \| STDOUT_LOGGING \| NO_LOGGING | 未设置 |

日志配置：

```XML
<settings>
  <!--不使用插件框架 system 控制台输出！ -->
...
![img](images/66d80c8986440.png)

###  ${}形式

`${}`形式传参，底层Mybatis做的是字符串拼接操作。

![img](images/66d80cba93108.png)
...
![img](images/66d81c1966497.webp)

另外还可以采用Mybatis默认机制进行参数输入
...
- 增删改操作返回的受影响行数（DML）：直接使用 int 或 long 类型接收即可
- 查询操作的查询结果（DQL）
...
> Mybatis 内部给常用的数据类型设定了很多别名。 以 int 类型为例，可以写的名称有：int、_int、integer、Integer、java.lang.Integer、Int、INT、INTEGER 等等。
...
**细节解释：**
  ...
  **resultType = "全限定符 ｜ 别名 ｜ 如果是返回集合类型，写泛型类型即可"**

别名问题：

> [typeAliases](https://mybatis.org/mybatis-3/zh_CN/configuration.html#typeAliases)
...
<!-- resultType属性：指定封装查询结果的Java实体类的全类名 类全限定符 | 别名 -->
<select id="selectEmployee" resultType="com.atguigu.mybatis.entity.Employee">
...
查询结果返回多个实体类对象，希望把多个实体类对象放在List集合中返回。此时不需要任何特殊处理，在resultType属性中还是设置实体类类型即可（返回集合泛型即可，List\<employee> -> employee）。
...
> [Lombok 注解及其用法](https://blog.shiguang666.eu.org/2024/09/05/e149f7f54e77/)
...
| -------- | ----------------------------------------------- | --------------------------------- |
| 对一     | `association`标签/`javaType`属性/`property`属性 | Mapper配置文件中的resultMap标签内 |
| 对多     | `collection标签`/`ofType`属性/`property`属性    | Mapper配置文件中的resultMap标签内 |
...
如果没有给接口中List类型的参数使用`@Param`注解指定一个具体的名字，那么在`collection`属性中默认可以使用`collection`或`list`来引用这个list集合。这一点可以通过异常信息看出来：
...
- 可以在sources下创建mapper接口包一致的文件夹结构存放mapper xml文件
    ...
    ![](images/66d9a811a5a88.png)

    ![](images/66d9a81158678.png)

## 插件和分页插件PageHelper

> [Mybatis-PageHelper 使用方法](https://github.com/pagehelper/Mybatis-PageHelper/blob/master/wikis/zh/HowToUse.md)
...
![image-20240905215810068](images/66d9b8f2a288c.png)

#### 使用  IDEA连接数据库

**连接数据库**

![img](images/66d9b92da8761.png)

**填写信息**

![img](images/66d9b93fb2347.png)

**展示库表**

![img](images/66d9b94e36eaf.png)

#### 逆向工程使用

**选择要逆向生成的表**

![img](images/66d9b95dcf9c4.png)

**填写实体类配置信息**

![image-20240905220340279](images/66d9ba3cc5b28.png)

**填写mapper文件配置信息**

![image-20240905220502057](images/66d9ba8ec44d3.png)

**查看生成结果**

![image-20240905220738824](images/66d9bb2bc7285.png)
